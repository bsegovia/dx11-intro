...
static const char qjulia[] =
 ...
 "{"
   ...
   "return i;"
 "}"
 "float h(inout float3 f,float3 c,float4 w,float n)"
 "{"
   "float i,r=0.f;"
   "i=n;"
   "while(i>=n&&r<3.)"
 ...
 "{"
   ...
   "return f;"
 "}"
 "float4 c(float3 f,float3 i,float4 x,float w,float3 r,float3 u,bool b)"
 "{"
   "const float4 n=float4(.3,.3,.3,0);"
   "float4 z;"
   "z=n;"
   "i=normalize(i);"
   "f=c(f,i);"
 ...
 "}"
 "[numthreads(4,64,1)]"
 "void c(uint3 i:SV_GroupID,uint3 z:SV_DispatchThreadID,uint3 n:SV_GroupThreadID,uint r:SV_GroupIndex)"
 "{"
   "float4 x=float4((float)z.x,(float)z.y,0.f,0.f);"
   "float2 w=float2(c_width,c_height);"
   "float d=min(w.x,w.y);"
   "float2 b=float2(.5f,.5f),u=(x.xy-b*w)/d*3.*zoom;"
   "float3 y=float3(1.5f,.5f,4.f),e=float3(0.f,0.f,4.f),m=float3(u.x,u.y,0.f);"
   "y=mul(float4(y,0.),rotation).xyz;"
   "e=mul(float4(e,0.),rotation).xyz;"
   "m=mul(float4(m,0.),rotation).xyz;"
   "float3 h=e,D=m-h;"
   "float4 l=c(h,D,c_mu,c_epsilon,e,y,c_selfShadow);"
   "f[z.xy]=l;"
 "}";
